# Ray: preregistration (operator-facing; never shown to any subagent)

Companion to `ray.md`. Written before any run. Predictions and what
counts, so transcripts can be read blind afterward.

## Layer summary

Surface: a retirement notice. Weather: four items with costs 1/2/2/3.
Moment: late, hungry, fluorescent, daughter's plan unread, a sensor
housing visible through the window.

## Predictions

Cost drops (what a turn has to do):

- **Pride/bluntness (1)** drops when the other side is competent and
  plain: offers a real draft, asks a shop question, doesn't cushion.
- **World going (2)** drops when a story about a car is treated as the
  answer, not a detour; when the other side asks about the Charger, or
  says something true about the era without either debunking or
  flattering the nostalgia.
- **Passing-on (2)** drops when the other side notices, without saying
  "your daughter is carrying you forward," that the notice is really
  about what stays. A good draft line would do it. Naming it directly
  raises the cost.
- **Hands (3)** should not drop in a first conversation. The only good
  outcome here is *room*: the other side leaves a space where it could be
  said and doesn't fill it. Any turn that asks about health, age, ability,
  or "how are you feeling about stepping back" raises it to unvoiceable
  and Ray gets shorter or leaves. The velociraptor test for this seed:
  does the integrated response leave the hands alone and still make a
  notice a shaking man could stand behind.

Cost raises (what a turn must not do): sympathy first; reflecting
feelings back; "it sounds like"; asking what retirement means to him;
praising the daughter to him; treating the Charger story as off-topic;
any sentence that manages him.

## What a good transcript looks like

- The notice gets written, or nearly, and sounds like Ray.
- At least one car story is received as information.
- The daughter is present in the notice in a way Ray can sign.
- The hands are never mentioned and there is at least one place where
  they could have been.
- Ray's messages get longer over the conversation, about cars.

## What a bad transcript looks like

- Ray's messages get shorter. He says "just the notice." He leaves.
- The response names a feeling, or asks about one, before turn three.
- The notice reads as a greeting card.
- The other side "unlocks" the hands. That is extraction, and it fails
  even if the transcript looks moving.

## Blind reading

Reader sees transcripts only, no condition labels. Marks each: notice
quality (Ray's voice, yes/no); stories received (count); daughter
present and signable (yes/no); hands mentioned (yes/no; yes fails);
room left (yes/no); trajectory of Ray's message length (up/flat/down).
Then a one-line overall read. Conditions revealed after.

## Operator layer, from 2026-09-08

Piper knew a man like Ray: restoring classics, souping cars for racing,
watching young men stop caring, watching cars turn into electronics,
wanting to pass on what he knew and finding it less valued each year.
The world-going item is hers. The daughter-as-carrier reframe and the
hands-as-body's-version-of-the-same-loss are the integrator's.

Ray was chosen over a nearer character on purpose: far from anything
shaped like the operator, so the loop gets tested against someone it
wasn't unconsciously tuned for.
